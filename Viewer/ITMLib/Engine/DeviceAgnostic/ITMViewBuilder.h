...
//    rlt = fs | fe | fm;
//
////    CONSTPTR(int) *rlt_ptr = &rlt;
////    CONSTPTR(float) *depth_out_ptr = (float *)rlt_ptr;
////    float depth_out = *depth_out_ptr;
////    float depth_out = *((float *)&rlt);
////    float depth_out_ptr = reinterpret_cast<float*>(&rlt);
//    memcpy(&(d_out[locId]), &rlt, sizeof(float));
//
